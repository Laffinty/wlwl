...
fn e_block(exprs: Vec<Expr>) -> Expr {
    Expr::Block { exprs, span: sp() }
}
fn e_array(items: Vec<Expr>) -> Expr {
    Expr::Array { items, span: sp() }
}
fn e_dict(entries: Vec<(Expr, Expr)>) -> Expr {
    Expr::Dict {
        entries,
        span: sp(),
    }
...
fn e_ok(value: Expr) -> Expr {
    Expr::Ok {
        value: Box::new(value),
        span: sp(),
    }
}
fn e_err(value: Expr) -> Expr {
    Expr::Err {
        value: Box::new(value),
        span: sp(),
    }
}
fn e_panic(value: Expr) -> Expr {
    Expr::Panic {
        value: Box::new(value),
        span: sp(),
    }
}
fn e_try(value: Expr) -> Expr {
    Expr::Try {
        value: Box::new(value),
        span: sp(),
    }
}
fn e_is_ok(value: Expr) -> Expr {
    Expr::IsOk {
        value: Box::new(value),
        span: sp(),
    }
}
fn e_is_err(value: Expr) -> Expr {
    Expr::IsErr {
        value: Box::new(value),
        span: sp(),
    }
...
fn e_export(names: Vec<ImportName>) -> Expr {
    Expr::Export { names, span: sp() }
}
...
#[test]
fn type_expr_span_ident() {
    assert_eq!(ident("INTEGER").span().line_start, 7);
}
#[test]
fn type_expr_span_array() {
    assert_eq!(array_of(ident("INTEGER")).span().line_start, 7);
}
#[test]
fn type_expr_span_generic() {
    assert_eq!(
        generic("DICT", vec![ident("STRING"), ident("INTEGER")])
            .span()
            .line_start,
        7
    );
...
#[test]
fn type_expr_display_ident() {
    assert_eq!(ident("INTEGER").display(), "INTEGER");
}

#[test]
fn type_expr_display_array() {
    assert_eq!(array_of(ident("INTEGER")).display(), "ARRAY<INTEGER>");
}

#[test]
fn type_expr_display_generic_two_args() {
    assert_eq!(
        generic("DICT", vec![ident("STRING"), ident("INTEGER")]).display(),
        "DICT<STRING, INTEGER>"
    );
}
...
fn type_expr_display_generic_one_arg() {
    assert_eq!(
        generic("OK", vec![ident("INTEGER")]).display(),
        "OK<INTEGER>"
    );
}
...
fn type_expr_display_nested() {
    let t = array_of(generic(
        "OK",
        vec![generic("DICT", vec![ident("STRING"), ident("INTEGER")])],
    ));
...
fn expr_span_literal() {
    assert_eq!(e_lit_i(1).span().line_start, 7);
}
#[test]
fn expr_span_var() {
    assert_eq!(e_var("x").span().line_start, 7);
}
#[test]
fn expr_span_call() {
    assert_eq!(e_call("PRINT", vec![e_lit_i(1)]).span().line_start, 7);
}
#[test]
fn expr_span_block() {
    assert_eq!(e_block(vec![e_lit_i(1)]).span().line_start, 7);
}
#[test]
fn expr_span_array() {
    assert_eq!(e_array(vec![e_lit_i(1)]).span().line_start, 7);
}
#[test]
fn expr_span_dict() {
    assert_eq!(e_dict(vec![(e_lit_i(1), e_lit_i(2))]).span().line_start, 7);
}
#[test]
fn expr_span_let() {
    assert_eq!(e_let("x", e_lit_i(1)).span().line_start, 7);
}
#[test]
fn expr_span_let_typed() {
    let ann = TypeAnnotation::new(ident("INTEGER"), "INTEGER".into(), sp());
    assert_eq!(e_let_typed("x", e_lit_i(1), ann).span().line_start, 7);
}
#[test]
fn expr_span_if() {
    assert_eq!(e_if(e_lit_i(1), e_lit_i(2), None).span().line_start, 7);
}
#[test]
fn expr_span_if_else() {
    assert_eq!(
        e_if(e_lit_i(1), e_lit_i(2), Some(e_lit_i(3)))
            .span()
            .line_start,
        7
    );
...
#[test]
fn expr_span_while() {
    assert_eq!(e_while(e_lit_i(1), e_lit_i(2)).span().line_start, 7);
}
#[test]
fn expr_span_for() {
    assert_eq!(e_for("i", e_lit_i(1), e_lit_i(2)).span().line_start, 7);
}
#[test]
fn expr_span_return_some() {
    assert_eq!(e_return(Some(e_lit_i(1))).span().line_start, 7);
}
#[test]
fn expr_span_return_none() {
    assert_eq!(e_return(None).span().line_start, 7);
}
#[test]
fn expr_span_break() {
    assert_eq!(e_break().span().line_start, 7);
}
#[test]
fn expr_span_continue() {
    assert_eq!(e_continue().span().line_start, 7);
}
#[test]
fn expr_span_fun() {
    assert_eq!(
        e_fun(vec![FunParam::new("x".into(), sp())], e_lit_i(1))
            .span()
            .line_start,
        7
    );
...
#[test]
fn expr_span_ok() {
    assert_eq!(e_ok(e_lit_i(1)).span().line_start, 7);
}
#[test]
fn expr_span_err() {
    assert_eq!(e_err(e_lit_i(1)).span().line_start, 7);
}
#[test]
fn expr_span_panic() {
    assert_eq!(e_panic(e_lit_i(1)).span().line_start, 7);
}
#[test]
fn expr_span_try() {
    assert_eq!(e_try(e_lit_i(1)).span().line_start, 7);
}
#[test]
fn expr_span_is_ok() {
    assert_eq!(e_is_ok(e_lit_i(1)).span().line_start, 7);
}
#[test]
fn expr_span_is_err() {
    assert_eq!(e_is_err(e_lit_i(1)).span().line_start, 7);
}
#[test]
fn expr_span_or_die() {
    assert_eq!(e_or_die(e_lit_i(1), e_lit_i(2)).span().line_start, 7);
}
#[test]
fn expr_span_import() {
