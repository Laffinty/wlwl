...
| P7-C2-001 | §4.4 row "E0056 \| SPAWN 中 fn 参数个数错误" | C2 commit `cce3ce3` 把 SPAWN 0 参数调用映射为 E0022 而非 plan 指定的 E0056。`builtin_spawn` 的 doc-comment 已正确引用 E0056,但实际 diag 调用走的是 E0022(与 SCOPE 的 arity check 同形)。 | 与 SCOPE 现有 E0022 arity check 模式保持一致;若不修,后续 AWAIT/YIELD/CHANNEL_* 沿用会传染,7 个 builtin arity 错误码分裂。 | 已修复(commit 见下) |
