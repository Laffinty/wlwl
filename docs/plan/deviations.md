...
| P7-E0-001 | §3 Phase E4 / §6.0 ERR consumer 跨 task 回归 registry 范围 | Phase E-A 的 8 个单元测试覆盖了**已注册**的 §8.3 ERR 消费者(BOOL / UNWRAP_OR / IS_ERR / UNWRAP / ERR_PAYLOAD / EXPECT_ERR / IF / TRY;PRINT_ERR 作为负例)。**未**登记 `wlwl-eval/src/registry.rs` 的 v0.7 新增 builtin(SPAWN / AWAIT / SCOPE / YIELD / TASK_CURRENT / TASK_IS_CANCELLED / CHANNEL_*),因为 plan §8.1 / D18 规定 spec 文件直到 Phase H1 才允许改动,而 registry 是附录 G 的单源真相(spec v0.4 附录 G 是 93 entries 的锁定值)。Phase E4 的"以 Phase E 启动时 registry.rs 中的实际注册表为准"在 v0.7 内只能指 spec v0.4 附录 G 范围内的条目;Phase H1 写 spec v0.7 附录 G 时一并补登。 | registry 的 93 entries 是 B11 锁测试(`b11_registry_count_matches_spec_table`)钉死的;为几个 v0.7 内置去动这个锁 + spec §18(spec 增补有严格流程)是不划算的耦合。E-A 测试在 EVAL 端落地,等 H1 时 registry 跟着新附录 G 一次性增补即可,E4 的"8 unit"项已闭合。 | 计划在 H1 修复 — 写 spec v0.7 附录 G 时把 v0.7 builtin 补入 `BUILTIN_REGISTRY`,b11 lock bump 到 v0.7 数(预计 100+ entries) |
