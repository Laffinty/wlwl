...
### 4.1 数值 / 4.2 字符串 / 4.3 布尔与空 / 4.4 数组 / 4.5 字典 / 4.6 函数字面量
...
### 5.1 一切都是表达式 / 5.2 函数调用 / 5.3 块表达式 / 5.4 求值顺序
...
### 6.1 变量绑定 / 6.2 重新绑定 / 6.3 作用域规则 / 6.4 提升
...
### 7.1 条件:`IF` / 7.2 循环:`WHILE` / 7.3 循环:`FOR` / 7.4 提前退出

见 v0.2 §7。`v0.3 不变`(DICT 遍历插入序已是 v0.2 明确)。

---

## 8. 函数

(与 v0.2 一致,§8.4 元数规则已是 v0.2 补全)

### 8.1 函数是一等公民 / 8.2 函数定义 / 8.3 函数调用 / 8.4 函数应用与元数 / 8.5 闭包
...
### 9.1 算术 / 9.2 比较 / 9.3 逻辑 / 9.4 真值规则

见 v0.2 §9。`v0.3 不变`(`+` 混合类型规则已是 v0.2 明确)。
...
### 10.1 数组(ARRAY) / 10.2 字典(DICT) / 10.3 字符串(STRING) / 10.4 相等性

见 v0.2 §10。`v0.3 不变`(DICT 遍历插入序已是 v0.2 明确)。
...
### 11.1 设计取向 / 11.2 类定义 / 11.3 实例化与属性 / 11.4 方法调用的链式语法糖 / 11.5 继承 / 11.6 封装 / 11.7 静态属性/方法
...
(与 v0.2 一致;§12.6 ERR 透明传播仍是 v0.3 核心)

### 12.1 范式 / 12.2 触发与传播 / 12.3 `TRY` 的关键语义 / 12.4 `PANIC` vs `ERR` / 12.5 错误信息结构 / 12.6 ERR 透明传播规则

见 v0.2 §12。`v0.3 不变`。**§19.6 给出 ERR 透明传播的形式化定义**。
...
> **v0.2 完整原文交叉引用**:见 `docs/history/20260902.md` 的 `# 后续版本归档:v0.2` 节。
