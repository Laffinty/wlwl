...
>
> **v0.8 关系(澄清性补丁 + 一项语法放宽)**:v0.8 在 v0.7 文本之上做 **clarification / documentation / alignment** 改动 —— §12 保留形式重写、§4.3 NOT 透传 prose 反向、§17.1 YIELD 位置限制降为实现路径、`BUILTIN_REGISTRY` 85 个 `section` 字段对齐 v0.7 章节号、`EXPECT_ERR` 入 §8.3 表、`%` 浮点 E0030 列入触发列表、`SHIELD` / `SCOPE(ERR)` / `AWAIT` 宿主诊断澄清、`=` 三重身份消歧、`LET` / `FUN` 不对称 normative、`TASK` 类型名 vs 命名空间成员同名不冲突 normative、§17.7 限制表 §0.1 版本约定补段。**唯一会改变可观察行为的边界场景**:字面量下标 `[1, 2, 3][0]`(v0.7 下解析错误,v0.8 起求值得 `1`)。任何 v0.7 下有定义、且**不依赖 §12 旧保留形式表**的程序,在 v0.8 下求值结果相同。v0.8 改动清单见 `CHANGELOG.md [v0.8.0]` 与 `docs/plan/deviations.md` 的 D8-001 至 D8-008。
