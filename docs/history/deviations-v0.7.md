...
> v0.7 实施偏差总账(含 v0.1–v0.6 累计条目)。原路径 `docs/plan/deviations.md`。
> 归档时改名为 `deviations-v0.7.md`,便于与未来版本分册。
> 配套计划:[`wlwl-build-plan-v0.7-COMPLETED.md`](wlwl-build-plan-v0.7-COMPLETED.md)。
> 新一轮迭代请在 `docs/plan/deviations.md` 另起空白登记,不要改写本文件。
...
| P7-B5a3-001 | §3 Phase B5a-3 完整 CPS(mid-body 挂起恢复) | **采用路径 B(任务体 CPS 切片)**,未走路径 A(eval 全栈机化,plan §5.1 写的那种)也未走路径 C(重跑 + yield 计数器)。YIELD 必须作为 multi-statement Block 的**直接子项**出现(FUN body / IF-then 分支等多语句 Block),LET RHS / 数组字面量 / 间接调用在 SPAWN 时直接 E0014 拒绝。Segment 在 SPAWN 时静态切好;Scheduler::step 每次只跑一段,遇到 `Signal::Yield(Explicit)` 保存 env 后让出。 | 三条候选路径的取舍见 2026-09-22 决策表。路径 A 改动太大(~600 LOC,触及 v0.6 同步路径,fidelity 风险高);路径 C 让 YIELD 之前的副作用会重跑(用户必须保证 idempotent,语义负担重);路径 B 严格 2-3 commit 范围,改动局限于 task body 边界,fidelity 不动。代价:不允许 YIELD 出现在"动态切片"位置(如 `IF(cond, YIELD(), 42)` 直接当 then 分支),但 IF 内多语句分支 `[YIELD()]` 等 Block 递归位置合法。 | —(路径 B 是终态,无后续修复需要) |
| P7-B5a3-002 | §3 Phase C5 builtin_yield | C5 + B5b 阶段,任务体内 `YIELD()` 走的是"返 NULL 不中断"的兼容绕道(`builtin_yield` 中 `if ev.current_task.is_some() { return Ok(Outcome::normal(Value::Null)) }`)。plan §3 Phase C4 描述的是真正的 mid-body suspend,该绕道只是 C5 测试期的占位。 | C5/B5b 时调度器还没接 mid-body,先把"不报错、不 yield"立住。 | 已修复(B5a-3-B:移除绕道,任务体内 YIELD 始终返 `Signal::Yield(Explicit)`,segment runner 捕获。`c5_yield_in_task_is_cooperative_checkpoint` 测试源同步更新为多语句 Block `1; YIELD(); 42`) |
| P7-D2-001 | §3 Phase D2/D3 `CHANNEL_SEND`/`CHANNEL_RECV` mid-body suspend | **CHANNEL_SEND 满 / CHANNEL_RECV 空 不挂起**,返 `ERR(kind="ChannelWouldBlock")`(D2/D3 与 §5.3 矩阵的 mid-body 列)。用户需用 `CHANNEL_TRY_SEND`/`CHANNEL_TRY_RECV`(或 buf > 0 + TRY_SEND)做非阻塞 back-pressure。 | 路径 B 的静态切段不能为"builtin 内部 yield"留位置:从 `Signal::Yield(SendingOn)` 冒到 segment runner 时,该 segment 已跑完,**没有"下一段"可跳**。要让它工作,要么重新跑 segment(会重做 pre-yield 的副作用,例如 `SEND 100` 会 push 两次),要么把 eval 改造成 plan §5.1 的栈机。前者是错的,后者是 path A 的 ~600 LOC 重写,v0.7.0 不值得。 | 计划在 path A / v0.7.1 修复;届时 `Signal::Yield` 在 builtin 内部也能保留 run_until_idle 的语义 |
| P7-D8-001 | §3 Phase D8 BOUNDED 通道死锁检测 | **v0.7.0 不做死锁检测**。D8 要求"调度循环检测所有任务均 blocked 且无 progress 的状态"——这要求 mid-body suspend 的精确挂起点,而 path B / P7-D2-001 已经把这一点退化成 ChannelWouldBlock ERR,所以死锁检测也跟着一并搁置。 | D8 与 P7-D2-001 同根——没有精确的 mid-body suspend 就没有"任务确实阻塞在某 channel"的概念。 | 计划在 path A / v0.7.1 修复 |
| P7-E0-001 | §3 Phase E4 / §6.0 ERR consumer 跨 task 回归 registry 范围 | Phase E-A 的 8 个单元测试覆盖了**已注册**的 §8.3 ERR 消费者(BOOL / UNWRAP_OR / IS_ERR / UNWRAP / ERR_PAYLOAD / EXPECT_ERR / IF / TRY;PRINT_ERR 作为负例)。**未**登记 `wlwl-eval/src/registry.rs` 的 v0.7 新增 builtin(SPAWN / AWAIT / SCOPE / YIELD / TASK_CURRENT / TASK_IS_CANCELLED / CHANNEL_*),因为 plan §8.1 / D18 规定 spec 文件直到 Phase H1 才允许改动,而 registry 是附录 G 的单源真相(spec v0.4 附录 G 是 93 entries 的锁定值)。Phase E4 的"以 Phase E 启动时 registry.rs 中的实际注册表为准"在 v0.7 内只能指 spec v0.4 附录 G 范围内的条目;Phase H1 写 spec v0.7 附录 G 时一并补登。 | registry 的 93 entries 是 B11 锁测试(`b11_registry_count_matches_spec_table`)钉死的;为几个 v0.7 内置去动这个锁 + spec §18(spec 增补有严格流程)是不划算的耦合。E-A 测试在 EVAL 端落地,等 H1 时 registry 跟着新附录 G 一次性增补即可,E4 的"8 unit"项已闭合。 | **已修复(H1, 2026-09-22)** — `BUILTIN_REGISTRY` 补登 17 个 v0.7 并发 builtin,`Version::V07` + `BuiltinGroup::Concurrent`;b11 lock bump **93 → 110**;`docs/appendix_G.md` 与 `docs/standard/wlwl-spec-v0.7.md` 附录 G 同步;§3.5 遮蔽保护现已覆盖 `SCOPE` 等新名(`LET(SCOPE,1)` → `E0025`) |
| P7-E3-001 | §3 Phase E3 SCOPE 兄弟取消 + §5.4.1 "scope 返 ERR(首个)" / §10 D9 取消拓扑 | 路径 B 下 children 同步执行,SCOPE 内兄弟取消的"中断在飞兄弟"语义不可观测:fn body 在 invoke_closure 调用栈里把所有 children 跑到 terminal 后才返回,SCOPE 此时才扫到第一个 child Done(Err)/Failed 并设 cancel_requested,但此时已无兄弟处于 Pending/Running 状态可被打断。E-B-1 的 `run_one_task` 入口检查点仍然必要 — 它是 TASK_CANCEL(h) 跨 task 直接设 flag 的唯一响应点,只是无法在 SCOPE 兄弟取消路径上触发"在飞兄弟"被中断。SCOPE 返回语义(plan §5.4.1 "scope 返 ERR(首个)")改成"fn body 自身 Value::Err 时透传 fn body ERR,其余情况不覆盖"(原意等价于"已消费过的 ERR 不再上浮,未消费的 ERR 已经在 fn body 的 Value::Err 里");路径 A 修复后可重新对齐 §5.4.1 矩阵"scope 取消该 task"那行的精确语义。 | 路径 B 的静态切段不能在 builtin 内留 yield 点,所以 SPAWN 的 child body 一旦进入,要么跑完,要么 E-C 通道挂起。`eb_scope_returns_fn_body_err_when_fn_body_has_uncaught_err` 单元测试 + `scope_cancel_siblings.wll` conformance fixture 已覆盖 ERR 透传路径(§5.4.1 cell "scope 返 ERR(首个)" 通过 fn body 的 Value::Err 落地);路径 B 全部消除了"在飞兄弟被中断"的可见性 → E3 行为在 v0.7.0 等价于 v0.6 §8.5 顶层逃逸 + §8.3 透明传播,只是多了一个跨兄弟的"剩余取消"标记位。 | 计划在 path A / v0.7.1 修复;届时 builtin 内部也能 yield,真正的 mid-body suspend 可以让 SCOPE 在 child 失败瞬间立即 cancel 在飞兄弟 |
| P7-E5-001 | §3 Phase E5 IF(cond, then, else) 在并发路径下的短路语义 | **无偏离**。评估结论:路径 B 下 IF 短路语义与 §8.3 / §6.1 完全一致,无需调整 `eval_if`(lib.rs:5860)。具体检查:(1) truthy 跨 task(`OK(42)` via AWAIT)→ then 分支;(2) falsy 跨 task(FALSE via AWAIT)→ else 分支;(3) ERR 条件 + else → else(已由 E-A 的 `e1_if_with_awaited_err_condition_routes_to_else` 覆盖);(4) ERR 条件 + no else → Outcome{value: Value::Err, signal: None},SCOPE.fn body 透传,top-level IS_ERR == TRUE(新 `ed_if_err_condition_no_else_surfaces_err_to_scope` 锁定);(5) THEN/ELSE 分支内的 SPAWN/AWAIT 同步执行(`ed_if_nested_with_concurrent_branches` 锁定 both arms reachable);(6) IF 内部不产生新 checkpoint,所以路径 B 同步约束无关。`&&`/`||` 的短路逻辑(`eval_logical_short_circuit`)与 IF 同根,亦不需调整。 | 5 个新增 `ed_*` 单元测试覆盖以上 6 个不变量;既有 `control_if_then` / `control_if_no_else` / `e1_if_with_awaited_err_condition_routes_to_else` 已锁定基本路径。Phase E5 评估结束,无需为 v0.7.0 改 plan §3 E5 文字或代码。 | —(IF 短路语义在 v0.7.0 / v0.7.1 / path A / path B 下一致,无后续修复需要) |
| P7-F5-001 | §3 Phase F5 SHIELD fn body 中"YIELD 后立即检查 pending cancel"语义 | 路径 B 下 SHIELD block 内的"YIELD 检查点"不可观测 — 同 P7-E3-001 / P7-D2-001(同步执行无在飞 checkpoint)。具体:SHIELD 设计要求"YIELD()/TASK_IS_CANCELLED() 在 SHIELD 退出后立即触发挂起的取消",而 path B 的 task body 在没有 YIELD 指令时是单段运行,所有 TASK_CANCEL_PARENT → TASK_IS_CANCELLED 在同一次 run_one_task 调用里完成;"退出后立即"的可观测等价是"同一个 fn body 内 SHIELD 之后的下一条语句的 TASK_IS_CANCELLED 返回 TRUE"(已经是 lib.rs `fb_shield_basic_records_pending_cancel` / `fb_shield_nested_three_layers` 锁定的语义)。F5 conformance 三个 fixture (`shield_basic` / `shield_nested` / `shield_then_error`) 也只覆盖这条可见性。 | "SHIELD 退出后立即生效"语义在 path B 下等价于"同 fn body SHIELD 之后语句立即观察 cancel_requested=true",与 F5 plan 描述一致;没有功能差异,只是描述层面 path B 缺一个 yield/checkpoint 触发的可见中间态。3 个 conformance fixture + 7 个 unit test 已覆盖所有可见面。 | 计划在 path A / v0.7.1 修复 — builtin 内部能 yield 后,SHIELD 内 YIELD/取消检查点可观测到 pending cancel 在中途激活的全过程 |
