...
>
> **对 v0.7 的关系(澄清性补丁 + 一项语法放宽)**:v0.8 在 **wlwl-spec-v0.7** 之上做 **clarification / documentation / alignment** 改动 —— §12 保留形式重写、§4.3 NOT 透传 prose 反向、§17.1 YIELD 位置限制降为实现路径、`BUILTIN_REGISTRY` 85 个 `section` 字段对齐 v0.7 章节号、`EXPECT_ERR` 入 §8.3 表、`%` 浮点 E0030 列入触发列表、`SHIELD` / `SCOPE(ERR)` / `AWAIT` 宿主诊断澄清、`=` 三重身份消歧、`LET` / `FUN` 不对称 normative、`TASK` 类型名 vs 命名空间成员同名不冲突 normative、§17.7 限制表 §0.1 版本约定补段。**唯一会改变可观察行为的边界场景**:字面量下标 `[1, 2, 3][0]`(v0.7 下解析错误,v0.8 起求值得 `1`)。任何 v0.7 下有定义、且**不依赖 §12 旧保留形式表**的程序,在 v0.8 下求值结果相同。v0.8 改动清单见 `CHANGELOG.md [v0.8.0]` 与 `docs/history/deviations-v0.8.md` 的 D8-001 至 D8-008。
...
**规范版本约定**(v0.8 起):规范正文采用 `major.minor` 双段。新引入内容追加 patch 段(如 `v0.7.1`)仅用于 errata,不另发规范文件。历史性"不承诺"行的 `[v0.7.0]` 等 `[vN.M.P]` 标记是版本快照,引用 v0.7 实现路径,后续修订不动;新写章节使用 `v0.8`(省略 patch 段)。
...
关键字不得用作标识符。`CLASS`、`NEW`、`THIS` 是**保留关键字**:当前实现中它们的求值产生诊断 `E0020`(第 12 章)。v0.8 起第 12 章重写后,这些关键字在 `BUILTIN_REGISTRY`(附录 G 的 `Oop` 组)中各有正式条目(实现路径: `LexerMacro` / `ResolvedBuiltin`),已不再是"保留未实现"的状态。
...
> **v0.8 实现注**:`lexer` 不消费前导符号(`wlwl-lexer/src/lib.rs::read_number` 只读裸数字);`+x` / `-x` 在 `parser.parse_expr` 的 `TokenKind::Minus` 分支里被改写为 `+(0, x)` / `-(0, x)`,再走 §4.3 二元运算路径。`NEG(x)` 是独立的 1 元内建(注册表 §4.3),仅按名调用可达,parser 不合成。可观察行为与"前导符号字面量"等价:同 INTEGER_MIN 边界、同 §2.2 溢出语义。
...
- `%(a, b)` 只接受整数:`a - /(a, b) * b`,符号与被除数一致;`b` 为零产生 `E1003`。任一实参是 `FLOAT` 时产生 `E0030`(类型错;v0.8 起列入 `%` 的触发列表,见 §11.2)。
...
- 下标链可挂在变量、调用、属性访问、**或数组/字典字面量之后**(v0.8 起;此前 §A.2 grammar 已允许,prose 与 parser 对齐)。`[1, 2, 3][0]` 与 `["a": 1]["a"]` 都是合法表达式,等价于 `INDEX_GET` 对字面量直接求值;`[[1,2][0], 3]` 同样合法。
...
> **v0.8 澄清(normative)**:`FUN(name(params), body)` 与 `LET(name, FUN(...))` 等价,二者表达式的值都是该函数值;`LET(name, value)` 的值是 `NULL`(绑定动作已完成,无值可传递)。`LET` 因此**不得**作为实参或运算符操作数(语义上是"声明语句"而非"值表达式")。如需在表达式位置产生绑定效果,请用 `LET MUT` + `SET` 序列或将值绑定到一个 `FUN` 表达式。
...
> **v0.8 追加**:作为 `ExprStmt` 被求值并被丢弃的表达式(附录 A.2 `Program` 的子句),其值若为 `ERR`,同样按本节(8.2)的顶层逃逸处理;即使该函数实参为非 `ERR`,顶层丢弃本身构成"无消费者"。
...
> **v0.8 追加 `{N}` 与 `{name}` 混用规则**:`{N}` 的 `N` 自 `0` 起,指 `FORMAT(template, args...)` 中 `args` 的第 `N` 项(template 是第 `-1` 项,不计入索引)。当模板同时含 `{name}` 与 `{N}`:
...
>
> 该行为由实现锁测试 `format_positional_refers_to_dict_arg_renders_it` 守住(`impl/crates/wlwl-std/src/format.rs:433-437`)。
...
| **[v0.7]** `E0055` | **保留** — v0.7 / v0.8 **无触发路径**(deviation D8-003):通道关闭信号由 `ERR(kind="ChannelClosed")` 字典载荷承担(§8.1 / §17.2),不经过原生错误码。注册仅为未来切换至原生码的版本预留。 |
| **[v0.7]** `E0056` | `SPAWN` 元数错误(含 `SPAWN` 自身元数、以及 `SPAWN` 的 `fn` 形参个数不为 0) |
| **[v0.7]** `E0057` | **保留** — v0.7 / v0.8 **无触发路径**(deviation D8-003):跨任务不可变单元格实际触发 `E0024`(与单任务路径统一,§17.4)。注册仅为未来专用码预留。 |
...
> **v0.8 宿主诊断澄清**:"宿主诊断"特指实现内部错误(如 `E0101` 栈溢出、底层通道句柄失效 `E0053` 等)。该类诊断在 `AWAIT` 处重新抛出,使用 `WlwlError` 的 trace 链把目标任务的失败帧关联起来;**用户 `ERR` 值**(`RESULT` 变体)按 §17.5 作为普通结果返回,不重新抛出。
...
**`YIELD` 位置(理想语义 vs v0.7 / v0.8 实现路径)**

`YIELD()` 的**理想语义**是:出现在任何表达式位置,均可在该点让出。**v0.7 / v0.8 参考实现**采用任务体静态切段调度(impl `wlwl-eval/src/yield_split.rs`),因此仅支持 `YIELD()` 出现在多语句 `Block`(`( a; b; … )` 或函数体 `;` 序列)的**直接子项**位置;其他位置产生 `E0014`。**该限制是 v0.7 / v0.8 实现路径的产物**,不是语言语义规则;未来版本(挂起式调度)可放宽而不视为破坏性修订。

下列位置在 v0.7 / v0.8 下产生 `E0014`:
...
**限制(非规范摘要,详见 §17.7)**:v0.7 / v0.8 的实现采用任务体静态切段;嵌套构造(`WHILE`/`FOR`/`IF` 分支)内部的 `YIELD` 在让步后**不**恢复嵌套体的剩余部分,而是继续外层 `Block` 的下一段(实现路径限制;未来版本可放宽)。
...
| `SHIELD(fn)` | `SHIELD(fn) -> v` | 调用 `fn` 并**屏蔽**屏蔽块内收到的取消请求。**不**创建新作用域,只是取消屏蔽的包装。 |

> **v0.8 SHIELD / SCOPE 正交性澄清**:`SHIELD(fn)` **不**创建新作用域,只是取消屏蔽的包装。它可以出现在任何位置;在顶层调用时,`SHIELD` 仅屏蔽来自当前任务的取消(顶层任务无取消源,故为 no-op)。`SHIELD` 与 `SCOPE` 是两个正交维度:作用域负责生命周期,屏蔽负责取消推迟。两者可独立组合(`SCOPE(FUN(() , SHIELD(...)))`)。
...
> **v0.8 SCOPE(ERR) 澄清**:`SCOPE(ERR("x"))` 中,实参 `ERR("x")` 在 §8.2 透明传播阶段先于 §17.1 参数类型检查(`fn` 必须为函数)生效,故结果是 `ERR("x")` 透传,**不**产生 `E0052`。`E0052` 仅在 §8.2 透明传播放过之后、参数被识别为非函数时触发。
...
| 死锁检测 | **[v0.7.0 不做]** |
| 嵌套 `YIELD` | **[v0.7 / v0.8 实现路径]** 嵌套构造(`WHILE`/`FOR`/`IF` 分支)内 `YIELD` 不恢复嵌套体剩余部分,而是继续外层 `Block` 的下一段。语言理想语义不要求此限制;未来版本可放宽(见 §17.1)。 |
| 尾部 `YIELD` | **[v0.7 / v0.8]** 任务体末尾的 `YIELD()` 使任务以 `NULL` 值结束;不与"未消费 `ERR`"语义冲突。 |
...
<!-- OOP (3 条) -->
| `CLASS` | `CLASS(name?, parent, members) -> CLASS` | n/a | ✔ | v0.2 | ✓ macro | parser -> `Expr::*` (§11 [占位;OOP 未实现]) |
| `NEW` | `NEW(cls, args...) -> INSTANCE` | n/a | ✔ | v0.2 | ✓ macro | parser -> `Expr::*` (§11 [占位;OOP 未实现]) |
| `THIS` | `THIS -> 当前实例` | n/a | ✔ | v0.2 | ✓ macro | parser -> `Expr::*` (§11 [占位;OOP 未实现]) |
<!-- 属性 / 方法 (3 条) -->
| `GET_PROP` | `GET_PROP(obj, k) -> v / E0037` | ❌ | ❌ | v0.2 | ✓ builtin | `resolve_builtin` (§11 [占位;OOP 未实现]) |
| `SET_PROP` | `SET_PROP(obj, k, v) -> NULL` | ❌ | ❌ | v0.2 | ✓ builtin | `resolve_builtin` (§11 [占位;OOP 未实现]) |
| `CALL_METHOD` | `CALL_METHOD(obj, m, args...) -> v` | ❌ | ❌ | v0.2 | ✓ builtin | `resolve_builtin` (§11 [占位;OOP 未实现]) |
<!-- 构造器 (2 条) -->
